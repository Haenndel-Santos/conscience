...
Surge daí o **Princípio da Dependência Não Linear de Regime**: pequenas variações em acoplamento, memória, energia, valoração ou recursividade podem reorganizar qualitativamente o campo integrado. Isso já é coerente com as simulações realizadas, que produziram saturação do acoplamento cérebro–corpo, thresholds e separação entre vigília, ansiedade, sono profundo e reflexo. 
...
Um teste mais exigente, porém, revela que essa discriminação bruta não deve ainda ser lida como confirmação do mecanismo que a teoria propõe. Höhn et al. (2024) mostram que, em banda larga, a complexidade de Lempel-Ziv e a inclinação do espectro de potência (1/f) carregam informação altamente sobreposta sobre o estado cerebral — ou seja, parte do que a complexidade capta pode ser apenas uma mudança na forma do espectro de potência entre vigília e sono, não integração diferenciada em sentido mais forte. [31] Testar isso diretamente nos dados deste projeto confirma a suspeita: depois de controlar estatisticamente pela inclinação espectral de cada época, a discriminação antes quase perfeita entre vigília e sono profundo cai para um patamar que não se distingue do acaso, e um índice construído especificamente para capturar integração×diferenciação — desenhado para superar uma medida de sincronia bruta — não o faz, nem antes nem depois desse controle. O achado bruto (vigília e REM mais complexos que o sono profundo) permanece de pé, robusto e replicado em dezenas de sujeitos; o que não resiste ao teste é a leitura de que essa diferença reflita, especificamente, integração diferenciada, e não apenas uma assinatura espectral mais simples e já conhecida. Isso é reportado com a mesma honestidade que orienta o restante deste capítulo: um resultado negativo para a interpretação mecanística é informação real sobre os limites atuais da teoria, não algo a esconder atrás do dado bruto que continua verdadeiro.

### Anestesia

Anestesia oferece um teste mais difícil do que o sono, e a primeira tentativa deste projeto falhou nele. Comparando vigília basal e sedação moderada por propofol sem distinguir entre sujeitos, a complexidade de sinal discriminou na direção oposta à prevista — a sedação parecendo, em média, mais complexa que a vigília basal. A resolução não veio de descartar o dado, mas de uma variável que o desenho original não usava: responsividade comportamental real, não apenas a dose nominal do fármaco. Separando os mesmos sujeitos por essa responsividade, reconstruída a partir do próprio conjunto de dados, o padrão se torna interpretável e replica um achado publicado de forma independente: [32] sujeitos que permanecem responsivos sob sedação moderada mostram um aumento *paradoxal* de complexidade, enquanto os que perdem a responsividade mostram a queda esperada — dose de fármaco e profundidade real da sedação não são a mesma variável. O episódio é, ele mesmo, uma ilustração do argumento central deste capítulo: a relação entre complexidade e consciência não é uma escala única aplicável a qualquer manipulação pelo rótulo experimental que ela recebe, mas depende do estado real, muitas vezes oculto, do sistema.
