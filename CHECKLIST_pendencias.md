...
10. **Repositório GitHub** — `main` e `revisao-2026-08` publicados em https://github.com/Haenndel-Santos/conscience (privado). Não abri PR de `revisao-2026-08` para `main` nem fiz merge — decidir se/quando quer isso.
11. **⚠️ Decisão de governança pendente — `PLANO_ESTRATEGICO_cientifico.md`** (commitado em `1eedb1b`, junto com os `PROMPT_*.md` — todos de origem presumida em outro agente/Codex trabalhando no mesmo projeto, não desta conversa). O plano propõe uma regra: *"agentes não executam cálculos/simulações — só escrevem scripts; o autor roda localmente"*. Isso **contradiz** como esta sessão trabalhou de fato (execução direta de downloads, análises de EEG, simulações). **Não adotei essa regra sozinho.** Precisa da sua decisão explícita: seguimos como até agora (eu executo), ou muda para o modo "só escreve, você roda"?
